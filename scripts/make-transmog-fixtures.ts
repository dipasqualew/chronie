...
for (const model of models) {
  for (const [suffix, bytes] of [
    ["m2", writeModel(model)],
    ["skin", writeSkin(model)],
  ] as const) {
    const path = join(OUT, `${suffix === "m2" ? model.fileDataId : model.skinFileDataId}.${suffix}`);
    writeFileSync(path, bytes);
    console.log(`${path}  ${bytes.length} bytes`);
  }
}

// Model 140007 is a file that is there and is not a model — a chunked file whose chunks are
// nothing this reader knows. An install missing a file is ordinary and shows the icon; a file
// that will not parse is this app being wrong about the format, and has to say so.
const unreadable = join(OUT, "140007.m2");
writeFileSync(unreadable, new Uint8Array(32));
console.log(`${unreadable}  32 bytes`);
