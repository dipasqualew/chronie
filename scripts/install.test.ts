...
const { productName } = JSON.parse(read("apps/desktop/src-tauri/tauri.conf.json")) as {
  productName: string;
};
...
function selectedSuffix(): string {
  ...
  if (!match?.[1]) {
    throw new Error(
      'install.ps1 no longer picks its release asset with a `-like "*suffix"` filter',
    );
  }
  return match[1];
}
...
function publishedArchiveName(): string {
  ...
  if (!match?.[1]) {
    throw new Error(
      "dev-release.yml no longer assigns the uploaded archive's name to $archiveName",
    );
  }
  return match[1];
}
...
describe("install.ps1", () => {
  ...
  it("registers an uninstall entry that points at the uninstaller it shipped", () => {
    expect(installer).toContain(
      String.raw`HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\Chronie`,
    );
