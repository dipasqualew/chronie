...
function backend(options: Options): void {
  ...
  execFileSync(
    "cargo",
    [
      "run", "--release", "--quiet", "--manifest-path", MANIFEST, "--example", "trace_render",
      "--", ...source, options.what, "--runs", String(options.runs),
      ...(options.reuse ? ["--reuse"] : []),
    ],
...
function model(options: Options): Buffer {
  ...
  execFileSync(
    "cargo",
    [
      "run", "--release", "--quiet", "--manifest-path", MANIFEST, "--example", "dump_model",
      "--", ...source, options.what, out,
    ],
