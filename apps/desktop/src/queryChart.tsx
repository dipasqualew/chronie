    ...
    >
      ...
      {plot.yTicks.map((tick) => (
        <g key={`y-${tick.label}-${tick.at}`}>
          <line className="chart-grid" x1={plot.frame.left} x2={right} y1={tick.at} y2={tick.at} />
          <text
            className="chart-tick chart-tick-y"
            x={plot.frame.left - LABEL_GAP}
            y={tick.at}
            textAnchor="end"
      ...
      <line className="chart-axis" x1={plot.frame.left} x2={right} y1={bottom} y2={bottom} />
      ...
      <text
        className="chart-axis-label"
        x={14}
        y={(plot.frame.top + bottom) / 2}
        textAnchor="middle"
        transform={`rotate(-90 14 ${(plot.frame.top + bottom) / 2})`}
      >
        {plot.yLabel}
      </text>
    </svg>
