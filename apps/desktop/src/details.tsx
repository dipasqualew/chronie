  ...
  return (
    <>
      ...
      {list.length > 2 ? (
        <>
          {" "}
          <span className="muted">+{list.length - 2} more</span>
        </>
      ) : null}
    </>
  );
...
const repText = (gains?: ReputationGain[]): string =>
  (gains || [])
    .map(
      (gain) =>
        `${gain.faction} +${gain.amount.toLocaleString()}` +
        (gain.standing ? ` (${gain.standing})` : ""),
    )
    .join(", ");
const currencyText = (gains?: CurrencyGain[]): string =>
  (gains || [])
    .map(
      (gain) =>
        `${gain.name} ${signed(gain.amount)}` +
        (gain.total == null ? "" : ` (${gain.total.toLocaleString()})`),
    )
    .join(", ");
const achievementText = (earned?: AchievementEvent[]): string =>
  (earned || [])
    .map((event) => `${event.name} (${event.accountFirst ? "account first" : "character first"})`)
    .join(", ");
...
const housingText = (events?: HousingItemEvent[]): string =>
  (events || [])
    .map((event) => `${event.name} (${event.warbandFirst ? "warband first" : "additional"})`)
    .join(", ");
const transmogText = (events?: TransmogEvent[]): string =>
  (events || [])
    .map(
      (event) =>
        `${itemName(event.id, event.name)} (${
...
const ALL_COLUMNS: Column[] = [
  ...
  },
  {
    ...
    cell: (segment) => (
      <>
        ...
        {segment.character}
        {segment.level == null ? null : (
          <>
            {" "}
            <span className="muted">Level {segment.level}</span>
          </>
        )}
      </>
    ),
  ...
  { key: "instance", title: "Location", cell: (s) => s.instance, sort: (s) => s.instance },
  {
    ...
    title: "Activity",
    always: true,
    cell: (s) => listCell(s.activities, activityText),
    sort: (s) => activityText(s.activities),
  },
  {
    key: "type",
    title: "Type",
    sort: (s) => locationType(s),
...
function withItemNames(segment: Segment, items: ItemBook): Segment {
  ...
  return {
    ...segment,
    transmogs: transmogs.map((event) =>
      event.name ? event : { ...event, name: items.detail(event.id)?.name || null },
    ),
  };
}
...
export function Details({ segments, onOpenSegment, items }: DetailsProps): ReactNode {
  ...
  const characters = useMemo(
    () => [...new Set(segments.map((entry) => entry.character))].sort(),
    [segments],
  );
  const days = useMemo(
    () => [...new Set(segments.map((entry) => entry.day))].sort().reverse(),
    [segments],
  );
  ...
  const rows = useMemo(() => {
    ...
    const filtered = segments.filter(
      (segment) =>
        ...
        (!day || segment.day === day) &&
        (!wanted ||
          `${segment.instance} ${segment.character} ${segment.difficulty}`
            .toLowerCase()
            .includes(wanted)),
    );
      ...
      <div className="table-head">
        <div className="controls">
          <input
            id="search"
            type="search"
            placeholder="Filter location or character…"
          ...
          <span className="count" id="count">
            {rows.length} of {segments.length} segments
          </span>
        </div>
      </div>
